      ...
      FO: FnMut(&Rhs<T, N, A>, Option<&T>) -> i32,
{
    ...
    stmts.push(cx.stmt_let_typed(DUMMY_SP, true, stack_id, stack_ty, quote_expr!(cx, ::std::vec::Vec::new())));
...
fn parse_parser<'a>(
    ...
    let r = try!(lr1_machine(
        cx, &grammar, &types, token_ty.clone(), span_ty, range_fn_id, range_fn, name,
        |&TerminalPattern { ident, unit_like }, cx| {
            let path = if let ast::TyKind::Path(_, ref path) = token_ty.node {
                let mut segments = path.segments.clone();
                segments.push(ast::PathSegment::from_ident(ident.0, DUMMY_SP));
                ast::Path { span: DUMMY_SP, segments: segments }
            } else {
                cx.path_ident(DUMMY_SP, ident.0)
            };
            cx.pat(DUMMY_SP,
            if unit_like {
                // `ident`
                ...
                // `ident(..)`
                ast::PatKind::TupleStruct(path, vec![], Some(0))
            })
        },
        |lhs, act, cx, syms| {
                    ...
                    Binding::Enum(sp, ref pats) => {
                        ...
                        let terminal = match *sym {
                            Nonterminal(..) => {
                                ...
                                ast::Path { span: sp, segments: vec![ast::PathSegment::from_ident(gensym("error"), DUMMY_SP)] }
                            }
                            Terminal(x) => if let ast::TyKind::Path(_, ref path) = token_ty.node {
                                let mut segments = path.segments.clone();
                                segments.push(ast::PathSegment::from_ident(x.ident.0, sp));
                                ast::Path { span: sp, segments: segments }
                            } else {
                                cx.path_ident(sp, x.ident.0)
                            }
                        };
                        expr = cx.expr_match(act.span, cx.expr_ident(sp, id), vec![
                            cx.arm(sp, vec![cx.pat(sp, ast::PatKind::TupleStruct(terminal, pats.clone(), None))], expr),
